...
Em geral, achamos a Allegro uma implementação boa e eficiente da parte gráfica, porém ao rodar o Valgrind tivemos alguns problemas de vazamento de memória referentes a
funções desta biblioteca que não é possível arrumar. \\
\\
Para essa fase retiramos a entrada de informações pela entrada padrão, as informações necessárias atribuímos no próprio código (hardcoded).
\\
